...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE year = 2014 and round = 'Eighth-Final' ORDER BY name ASC;")"
...
echo "$($PSQL "SELECT year, name FROM games INNER JOIN teams ON winner_id = team_id WHERE round =
'Final'ORDER BY year;")"
